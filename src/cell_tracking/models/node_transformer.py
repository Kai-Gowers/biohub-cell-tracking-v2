"""Transformer operating on nodes to predict edges between nodes (ported).

Ported verbatim from the public pilkwang ``biohub-tracking-support-pack``
(RoyerLab Kaggle baseline), ``src/biohub_tracking/models/simple_node_transformer.py``,
retrieved 2026-09-11 (``context/pack_primary``). No functional changes.
"""
...
class SimpleNodeTransformer(nn.Module):
    ...
    def forward(
        ...
        # Build pairwise logits in chunks over N_t to avoid O(N^2) peak allocation.
        N_t = q.shape[1]
