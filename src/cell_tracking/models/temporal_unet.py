"""Simple 3D+T U-Net (ported).

Ported verbatim from the public pilkwang ``biohub-tracking-support-pack``
(RoyerLab Kaggle baseline), ``src/biohub_tracking/models/temporal_unet.py``,
retrieved 2026-09-11 (``context/pack_primary``). The only change is the
attention backend guard in ``_TemporalAttention.forward``: the per-voxel
attention flattens ``B * Z * Y * X`` rows into the batch dimension (~262k at
the first attended stage), which crashes the fused SDPA kernels on L40S with
torch 2.6 (reports/2026-09-09-sdpa-math-backend-l40s.md), so the MATH backend
is forced. Numerics are the same up to floating-point kernel differences.
