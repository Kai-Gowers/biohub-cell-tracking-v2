...
Writes dist/error_examples/probe2.pkl: per miss, nearest peak distance under each rule; per frame, peak counts."""
import pickle, time
...
for k, (key, idxs) in enumerate(sorted(by_frame.items())):
    ...
    for lab, field, thr, r in (("logit_r2", logits, logit_tau, 2), ("prob_r1", prob, TAU, 1), ("logit_r1", logits, logit_tau, 1)):
        idx, _ = local_maxima(field, threshold=thr, radius=r)
        variants[lab] = grid_to_voxels(idx.numpy().astype(np.float64))
...
pickle.dump(dict(misses=out, frames=frames_out), open("dist/error_examples/probe2.pkl", "wb"))
